...
//    }
    ...
    public  Mono<GenericResponse<CustomerDto>> getCustomerById(Integer id){
        return this.client
//                .mutate().webClient(builder -> builder.).build()
                .documentName("customer-by-id")
                .variable("id", id)
                .execute()
                .map(clientGraphQlResponse -> {
                    var field = clientGraphQlResponse.field("customerById");
    ...
    public  Mono<CustomerResponse> getCustomerByIdWithUnion(Integer id){
        return this.client.documentName("customer-by-id")
                ...
                .map(clientGraphQlResponse -> {
                    var field = clientGraphQlResponse.field("customerById");
                    var isCustomer = "Customer".equals(clientGraphQlResponse.field("customerById.type").getValue().toString());
